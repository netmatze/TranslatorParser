...
namespace TranslatorParser
{
    ...
    public static class Parser
    {
        ...
        public static Parse<string> StringValueEmpty()
        {
            return value =>
                {
                    if (value == "")
                    {
                        return new ParseResult<string>(String.Empty, value, true);
                    }
                    else
                    {
                        return new ParseResult<string>(String.Empty, value, false);
                    }
                };
        }
        ...
        private static Parse<string> DecimalStringHelper(string x, bool front, int stelle)
        {
            return value =>
            {
                if (front)
                {
                    ...
                    if (parseResult.Succeeded)
                    {
                        string str = System.Convert.ToString(10 * System.Convert.ToInt32(x) + System.Convert.ToInt32(parseResult.Result));
                ...
                else
                {
                    ...
                    if (parseResult.Succeeded)
                    {
                        ...
                        string str = System.Convert.ToString(System.Convert.ToDecimal(x) + System.Convert.ToDecimal(dec));
        ...
        public static Parse<string> Literal(string literalToParse)
        {
            ...
            else
            {
                return value =>
                {
                    if (value != String.Empty && value.TrimStart(' ').Substring(0, 1) == localLiteralToParse)
                    {
                        return new ParseResult<string>(localLiteralToParse.Substring(0, 1), value.TrimStart(' ').Substring(1), true);
                    }
                    else
                    {
